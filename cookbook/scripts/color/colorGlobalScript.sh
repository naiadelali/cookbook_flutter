...
OUTPUT_COLOR_SCHEME_FILE="$OUTPUT_DIR/color.dart"
...
for group in $(jq -r '.ref.color | keys[]' "$JSON_FILE"); do

  # Processa cada chave dentro do grupo atual
  jq -r ".ref.color[\"$group\"] | keys[]" "$JSON_FILE" | while read -r key; do
    original_value=$(jq -r ".ref.color[\"$group\"][\"$key\"].value // empty" "$JSON_FILE")
    formattedKey="$(lower_first_char $group)$(upper_first_char $key)"
    ...
    if [[ "$original_value" =~ ^\{ref\.color\.([a-zA-Z0-9_]+)\.([a-zA-Z0-9_]+)\}$ ]]; then
      ...
      echo "final $formattedKey = $ref_value;" >> "$OUTPUT_COLOR_SCHEME_FILE"
    else
      ...
      echo "final $formattedKey = const Color(0xFF$hex_value);" >> "$OUTPUT_COLOR_SCHEME_FILE"
    fi
  done
  echo "" >> "$OUTPUT_COLOR_SCHEME_FILE"
done
